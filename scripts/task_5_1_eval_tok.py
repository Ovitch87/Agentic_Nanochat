...
for vocab_size in [8192, 32768]:
    ...
    print(
        f"{'Text':<16} {'Characters':>12} "
        f"{'Tokens':>12} {'Tokens/character':>18}"
    )
    ...
    for name, text in all_text:
        ...
        ratio = token_count / characters

        print(
            f"{name:<16} {characters:>12} "
            f"{token_count:>12} {ratio:>18.4f}"
        )

        examples = [
    ("numbers", "1234567890 123.45 -123.45 2026-09-17"),
    ("code", "def calculate_total(item_prices):\n    return sum(item_prices)"),
    ("Dutch", "De onderzoekers vergelijken verschillende resultaten."),
    ("Korean", "정직한 사실"),
]
...
for vocab_size in [8192, 32768]:
    ...
    for name, text in examples:
        ...
        print(f"Exact reconstruction: {tokenizer.decode(tokens) == text}")
