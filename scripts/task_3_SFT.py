...
from nanochat.checkpoint_manager import save_checkpoint, load_model_from_dir, load_optimizer_state
...
parser.add_argument("--max-seq-len", type=int, default=2048, help="max context length (default: inherit from pretrain)")
parser.add_argument("--device-batch-size", type=int, default=8, help="per-device batch size (default: inherit from pretrain)")
parser.add_argument("--total-batch-size", type=int, default=262144, help="total batch size in tokens (default: inherit from pretrain)")
...
model, tokenizer, meta = load_model_from_dir(
    os.path.abspath("task_3_results/chatsft_checkpoints"),
    device,
    phase="train",
    model_tag="d4_mid",
...
while True:
    ...
    if last_step:
        checkpoint_dir = os.path.abspath(f"task_3_results/chatsft_checkpoints/d{depth}_sft")
